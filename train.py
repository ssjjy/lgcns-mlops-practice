...
sys.excepthook = handle_exception  # try except없이 오류난 부분 로깅해줌 **
...
if __name__ == "__main__":
    ...
    logger.info("Running a preprocessing pipline...")
    # TODO: X=_X, y=y로 전처리 파이프라인을 적용해 X에 저장
    X = preprocess_pipeline.fit_transform(X=_X, y=y)

    logger.info("Saving a features data into the storage...")
    ...
    experiment_name = "new_experiment1"  # 이미 한번 생성됐으면 에러발생하기때문에 이름 바꿔줘야함
